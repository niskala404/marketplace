@extends('layouts.market')

@section('content')
<div class="flex items-center justify-between mb-4">
    <div>
        <h1 class="text-2xl font-black">Verifikasi Pembayaran</h1>
        <div class="text-sm text-slate-500">Daftar pesanan transfer manual yang menunggu verifikasi</div>
    </div>
    <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 rounded-xl bg-slate-900 text-white">Kembali</a>
</div>

<div class="bg-white border rounded-2xl overflow-hidden">
    <div class="divide-y">
        @forelse($orders as $o)
            <div class="p-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div>
                    <div class="font-bold">{{ $o->order_no }}</div>
                    <div class="text-sm text-slate-500">Pembeli: {{ $o->user->name }} • {{ $o->user->email }}</div>
                    <div class="text-sm text-slate-500">Toko: {{ $o->shop->name }}</div>
                    <div class="mt-2 font-black text-rose-600">Rp {{ number_format($o->grand_total,0,',','.') }}</div>
                    <div class="text-sm">Status: <span class="font-semibold">{{ $o->status }}</span></div>
                </div>

                <div>
                    @if($o->payment_proof_path)
                        <img class="w-full max-w-md rounded-2xl border" src="{{ asset('storage/'.$o->payment_proof_path) }}" alt="Bukti transfer">
                    @else
                        <div class="text-sm text-slate-600">Belum ada bukti transfer.</div>
                    @endif
                </div>

                <div class="flex flex-col gap-2">
                    <form method="POST" action="{{ route('admin.payments.verify', $o) }}">
                        @csrf
                        <button class="w-full px-4 py-3 rounded-xl bg-emerald-600 text-white font-bold">Verifikasi (PAID)</button>
                    </form>
                    <form method="POST" action="{{ route('admin.payments.reject', $o) }}">
                        @csrf
                        <button class="w-full px-4 py-3 rounded-xl bg-rose-600 text-white font-bold">Tolak Bukti</button>
                    </form>
                    <div class="text-xs text-slate-500">Catatan: detail item bisa dilihat via panel seller (order terkait toko) atau buat halaman detail admin (tahap berikutnya).</div>
                </div>
            </div>
        @empty
            <div class="p-6 text-slate-600">Tidak ada pembayaran yang menunggu verifikasi.</div>
        @endforelse
    </div>
</div>

<div class="mt-4">{{ $orders->links() }}</div>
@endsection
