...
<div class="max-w-6xl mx-auto space-y-4">
    ...
    <x-ui.card class="bg-gradient-to-r from-rose-50 to-white border-rose-200">
      <div class="flex items-center gap-3">
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-rose-600">
          <x-ic name="tag" class="w-5 h-5 text-white" />
        </span>
        <div class="min-w-0">
    ...
    <x-ui.card class="bg-gradient-to-r from-emerald-50 to-white border-emerald-200">
      <div class="flex items-center gap-3">
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-emerald-600">
          <x-ic name="truck" class="w-5 h-5 text-white" />
        </span>
        <div class="min-w-0">
    ...
    <x-ui.card class="bg-gradient-to-r from-slate-50 to-white">
      <div class="flex items-center gap-3">
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-slate-900">
          <x-ic name="shield-check" class="w-5 h-5 text-white" />
        </span>
        <div class="min-w-0">
        ...
        @foreach($flashItems as $it)
          ...
          <a href="{{ route('product.show', $p->slug) }}" class="block rounded-2xl border overflow-hidden hover:shadow-sm transition bg-white">
            <div class="relative aspect-[4/3] bg-slate-100">
              ...
              {{-- Per-card countdown (before click) --}}
              <div
            ...
            <div class="p-3">
              ...
              {{-- Sold badge + animated progress (Shopee feel) --}}
              <div class="mt-2">
                ...
                @if($pct !== null)
                  <div class="mt-1 h-3 rounded-full bg-rose-100 overflow-hidden">
                    <div class="h-full bg-rose-500 fs-progress rounded-full" style="width: {{ $pct }}%"></div>
                  </div>
                @else
                  <div class="mt-1 h-3 rounded-full bg-rose-100 overflow-hidden">
                    <div class="h-full bg-rose-500 fs-progress rounded-full" style="width: {{ min(100, max(5, (int)round(($sold % 20) / 20 * 100))) }}%"></div>
                  </div>
                @endif
              </div>
            </div>
  ...
  {{-- Kategori + Search + Filter (digabung jadi 1 container) --}}
  <div class="mt-4">
    <x-ui.card class="bg-white/95 backdrop-blur shadow-sm">
      ...
      <div class="mt-3 rounded-2xl border bg-white/90 px-3 py-2">
        <div class="flex items-center gap-2">
          <form id="searchForm" data-ajax-search action="{{ route('home') }}" method="GET" class="flex-1">
            {{-- keep params --}}
            @if($category)
          ...
          >
            ...
            <span id="filterBadge" class="absolute -top-2 -right-2 inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full bg-rose-600 text-white text-[11px] font-black {{ $activeFilterCount > 0 ? '' : 'hidden' }}">
              {{ $activeFilterCount }}
            </span>
          </button>
      ...
      {{-- Grid kategori bergambar (sekalian category selector AJAX) --}}
      <div class="mt-4">
        <div id="catChips" class="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-3">
          ...
          <a href="{{ route('home', array_filter(['q' => $q])) }}"
             ...
             data-cat-id=""
             data-cat-name="Semua"
             class="group flex flex-col items-center text-center gap-2 p-2 rounded-2xl border transition {{ !$category ? 'bg-slate-900 text-white border-slate-900' : 'hover:bg-slate-50' }}">
            ...
            <div data-cat-label class="text-[11px] leading-tight line-clamp-2 min-h-[28px] {{ !$category ? 'text-white' : 'text-slate-700 group-hover:text-rose-700' }}">
              Semua
            </div>
          </a>

          @foreach($categories as $cat)
            <a href="{{ route('home', array_filter(['q' => $q, 'category' => $cat->id])) }}"
               data-cat-chip
               data-cat-id="{{ $cat->id }}"
               data-cat-name="{{ $cat->name }}"
               class="group flex flex-col items-center text-center gap-2 p-2 rounded-2xl border transition {{ (string)$category === (string)$cat->id ? 'bg-slate-900 text-white border-slate-900' : 'hover:bg-slate-50' }}">
              <div data-cat-icon-wrap class="w-14 h-14 rounded-full border overflow-hidden flex items-center justify-center {{ (string)$category === (string)$cat->id ? 'bg-slate-800 border-slate-700' : 'bg-slate-100' }}">
                ...
                @else
                  <span data-cat-fallback class="font-black {{ (string)$category === (string)$cat->id ? 'text-white/80' : 'text-slate-400' }} text-lg">
                    {{ strtoupper(mb_substr($cat->name,0,1)) }}
                  </span>
                @endif
              </div>
              <div data-cat-label class="text-[11px] leading-tight line-clamp-2 min-h-[28px] {{ (string)$category === (string)$cat->id ? 'text-white' : 'text-slate-700 group-hover:text-rose-700' }}">
                {{ $cat->name }}
              </div>
            </a>
          @endforeach
  ...
  @else
    ...
    <div class="mt-4">
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        ...
        @foreach($tr as $kw)
          <a href="{{ route('home', array_merge($baseParams, ['q' => $kw])) }}" class="text-xs px-3 py-1.5 rounded-full border bg-white hover:bg-slate-50">
            {{ $kw }}
          </a>
        @endforeach
      </div>
  ...
  <div class="max-w-md w-full">
    <div class="rounded-2xl bg-slate-900 text-white px-4 py-3 shadow-lg flex items-center justify-between gap-3">
      ...
      <button type="button" id="toastClose" class="p-1.5 rounded-xl hover:bg-white/10" aria-label="Tutup">
        <x-ic name="x" class="w-5 h-5 text-white" />
      </button>
    </div>
  </div>
</div>
    ...
    <div class="p-4">
      <div class="flex items-center justify-between">
        ...
        <button type="button" id="sheetClose" class="p-2 rounded-xl hover:bg-slate-100">
          <x-ic name="x" class="w-6 h-6 text-slate-700" />
        </button>
      </div>
...
{{-- Storefront UX: skeleton + toast + quick add + infinite scroll --}}
<script>
(function(){
...
{{-- Filter behavior (desktop dropdown + mobile bottom-sheet) --}}
<script>
(function(){
  const btn = document.getElementById('filterBtn');
  if(!btn) return;

  const activeCount = {{ (int)$activeFilterCount }};
