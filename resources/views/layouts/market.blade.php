...
(function () {
    ...
    let toastTimer = null;
    let lastNotifCount = @json(auth()->check() ? auth()->user()->unreadNotifications()->count() : 0);
    ...
    async function checkNotifications() {
        @auth
        try {
            const res = await fetch(@json(route('notifications.check')), {
                headers: {
