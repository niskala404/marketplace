...
<div class="max-w-3xl mx-auto">
  ...
  <div class="sticky top-0 z-10 bg-white/85 backdrop-blur border rounded-t-2xl px-4 py-3 flex items-center gap-3">
    ...
    <div class="flex-1 min-w-0">
      <div class="font-black truncate">Chat: {{ $buyerName }}</div>
      <div class="text-xs text-slate-500 truncate">Balas pembeli di sini.</div>
    </div>
    <a href="{{ route('seller.messages.index') }}" class="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-sm font-semibold">Kembali</a>
  </div>
  ...
  <div class="bg-white border border-t-0 rounded-b-2xl overflow-hidden">
    ...
    <div class="border-t bg-white px-3 py-3">
      <form method="POST" action="{{ route('seller.messages.send',$conversation) }}" class="flex items-end gap-2" id="sendForm">
        @csrf
...
(function(){
  ...
  const pollUrl = @json(route('seller.messages.poll', $conversation));
