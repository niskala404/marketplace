...
<div class="max-w-3xl mx-auto">
  <?php
    $buyerName = $conversation->buyer->name ?? 'Pembeli';
  ...
  <div class="sticky top-0 z-10 bg-white/85 backdrop-blur border rounded-t-2xl px-4 py-3 flex items-center gap-3">
    ...
    <div class="flex-1 min-w-0">
      <div class="font-black truncate">Chat: <?php echo e($buyerName); ?></div>
      <div class="text-xs text-slate-500 truncate">Balas pembeli di sini.</div>
    </div>
    <a href="<?php echo e(route('seller.messages.index')); ?>" class="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-sm font-semibold">Kembali</a>
  </div>
  ...
  <div class="bg-white border border-t-0 rounded-b-2xl overflow-hidden">
          ...
          <div class="max-w-[82%]">
            <?php if($newGroup): ?>
              <div class="text-[11px] mb-1 <?php echo e($mine ? 'text-right text-slate-500' : 'text-left text-slate-500'); ?>">
                <?php echo e($mine ? 'Kamu' : $buyerName); ?>
    ...
    <div class="border-t bg-white px-3 py-3">
      <form method="POST" action="<?php echo e(route('seller.messages.send',$conversation)); ?>" class="flex items-end gap-2" id="sendForm">
        <?php echo csrf_field(); ?>
...
(function(){
  ...
  const pollUrl = <?php echo json_encode(route('seller.messages.poll', $conversation), 512) ?>;
...
<?php echo $__env->make('layouts.market', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\ilmishop\resources\views/seller/messages/show.blade.php ENDPATH**/ ?>