...
class CheckoutController extends Controller
{
    ...
    /**
     * Sanitize cart items: remove inactive products, mismatched SKUs, etc.
     * Returns a clean Collection ready to use.
     */
    private function sanitizeItems(Cart $cart)
    {
        ...
        if ($invalidIds) {
            ...
            return $cart->items()->with('product.images', 'product.shop', 'product.variants', 'variant')->get();
        }
    ...
    public function show(Request $request, ShippingCalculator $shipping, VoucherService $vouchers, CartPricingService $pricing)
    {
        ...
        if ($addresses->isEmpty()) {
            return redirect()->route('account.addresses.create')
                ->with('error', 'Tambahkan alamat dulu untuk checkout.');
        }
        ...
        // ✅ FIX: $groups->map() closure body was completely missing
        $shopSummaries = $groups->map(function ($groupItems, $shopId) use ($pricing, $shipping, $selectedAddress, $flashPriceMap) {
            $shop        = $groupItems->first()->product->shop;
            $groupItems  = $groupItems->values();

            $subtotal = (int) $groupItems->sum(function ($it) use ($pricing, $flashPriceMap) {
    ...
    public function place(Request $request, ShippingCalculator $shipping, VoucherService $vouchers, CartPricingService $pricing)
    {
        ...
        DB::transaction(function () use ($user, $items, $address, $groups, $request, $cart, $shipping, $vouchers, $platformVoucherCode, $shopVoucherCodes, $flashPriceMap, $pricing) {
            ...
            // 1) Pre-calc per shop: subtotal, shipping, shop voucher
            $calc = [];
            foreach ($groups as $shopId => $shopItems) {
                $shopId = (int) $shopId;

                // ✅ FIX: subtotal calculation was missing
                $subtotal = (int) $shopItems->sum(function ($it) use ($pricing, $flashPriceMap) {
                    $unit = $pricing->resolveUnitPrice($it->product, $it->variant, $flashPriceMap);
                    return $unit * (int) $it->qty;
                });
            ...
            // 2) Choose ONE platform voucher target (shop with highest applicable discount)
            ...
            foreach ($groups as $shopId => $shopItems) {
                ...
                if ($platformTargetShopId !== null && $shopId === $platformTargetShopId && $platformVoucherModel && $platformTargetDiscount > 0) {
                ...
                $order->logShipmentEvent('pending', 'Pesanan dibuat', 'Pesanan berhasil dibuat dan menunggu proses pembayaran.', now(), 'order_created');

                if ($order->status === 'processing') {
                    $order->logShipmentEvent('processing', 'Pesanan diproses', 'Pembayaran COD, pesanan langsung diproses penjual.', now(), 'processing');
                }
                ...
                // Redeem platform voucher (only for the chosen target shop)
                if ($platformTargetShopId !== null && $shopId === $platformTargetShopId && $platformVoucherModel && $platformTargetDiscount > 0) {
                ...
                foreach ($shopItems as $it) {
                    ...
                    // Increment sold count for flash sale item
                ...
                // Auto thank-you message from shop to buyer
                if ($shop?->user) {
    ...
    public function confirmReceived(Request $request, Order $order)
    {
        ...
        if ($order->shop?->user && $old !== $order->status) {
            $order->shop->user->notify(new OrderStatusChangedNotification($order, $old, $order->status));
        }
    ...
    public function cancel(Request $request, Order $order, VoucherService $vouchers)
    {
        ...
        DB::transaction(function () use ($order, $vouchers) {
            ...
            // ✅ FIX: broken foreach — variant stock restoration was missing
            foreach ($locked->items as $item) {
                Product::query()->whereKey($item->product_id)->increment('stock', (int)$item->qty);

                if ($item->product_variant_id) {
                    ProductVariant::query()->whereKey($item->product_variant_id)->increment('stock', (int)$item->qty);
                }
            }
